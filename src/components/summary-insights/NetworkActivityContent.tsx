...
const NETWORK_COLUMN_WIDGET_ORDER = ["traffic", "severity", "pairs"] as const;
...
export function NetworkActivityContent() {
  ...
  const [trafficViz, setTrafficViz] = useState<CategoricalWidgetViz>("bar");
  ...
  const columnExpand = useExpandableColumnWidgets(NETWORK_COLUMN_WIDGET_ORDER);
  ...
  }, [timeframeScopedRows, trafficFilter, severityFilters, pairFilter]);
  ...
  }, [timeframeScopedRows, trafficFilter, pairFilter]);
  ...
  const trafficChartRows = useMemo(
    () =>
      countByLabel(chartScopedRows, TRAFFIC_ORDER, (row) => row.trafficType).map((row) => ({
        ...row,
  ...
  const trafficSegments = useMemo(
    () => withCategoricalColors(trafficChartRows.map(({ label, value }) => ({ label, value }))),
    [trafficChartRows],
  );
  ...
  const severityChartRows = useMemo(
    () =>
      SEVERITY_CHART_ORDER.map((label) => {
        ...
        const value = chartScopedRows.filter((row) => row.severity === severityKey).length;
        return {
          label,
          value,
  ...
  return (
    <div className="flex shrink-0 flex-col gap-4 p-4 sm:p-5">
      ...
      <InsightCard
        title={`Network Events ${eventsPerHourChart.titleCadence} By Severity`}
        headerActions={
      ...
      >
        ...
        <TimeSeriesAreaChart
          ...
          selectedSeriesIds={normalizeSeverityFilterIds(severityFilters).filter(isNetworkSeverity)}
            ...
            id === "traffic" ? (
              trafficViz === "donut" ? (
                <DonutChartPanel
                  ...
                  onSegmentClick={handleTrafficClick}
                  ariaLabel="Traffic by activity type"
                />
              ) : (
                <HorizontalBarPanel
              ...
              severityViz === "donut" ? (
                <DonutChartPanel
                  ...
                  ariaLabel="Network activity by severity"
                />
              ) : (
                <HorizontalBarPanel
          ...
          const visualization =
            id === "traffic"
              ? {
                  ...
                  onChange: (next: string) => setTrafficViz(next as CategoricalWidgetViz),
                }
              : id === "severity"
                ? {
                    value: severityViz,
